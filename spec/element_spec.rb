...
RSpec.describe XsdModel::Elements::Element do
  describe '#name' do
    it "returns element's name" do
      element = Element.new(attributes: { 'name' => 'order' })
  ...
  describe '#type' do
    it "returns element's type" do
      element = Element.new(attributes: { 'type' => 'xs:string' })
  ...
  describe '#xsd_prefix' do
    it 'returns prefix associated with http://www.w3.org/2001/XMLSchema namespace' do
      element = Element.new(namespaces: { 'xs' => 'http://www.w3.org/2001/XMLSchema' })
      expect(element.xsd_prefix).to eq 'xs'

      element = Element.new(namespaces: { 'xsd' => 'http://www.w3.org/2001/XMLSchema' })
  ...
  describe '#has_custom_type?' do
    it "returns true if element's type is custom defined one" do
      element = Element.new(attributes: { 'type' => 'orderType' }, namespaces: { 'xs' => 'http://www.w3.org/2001/XMLSchema' })
      expect(element.has_custom_type?).to eq true
    end

    it "returns false if element's type is one of basic XSD types" do
      element = Element.new(attributes: { 'type' => 'xs:string' }, namespaces: { 'xs' => 'http://www.w3.org/2001/XMLSchema' })
  ...
  describe '==' do
    it 'returns true if elements attributes and childrens match' do
      element1 = Element.new(Text.new, attributes: { 'key' => 'value' })
      element2 = Element.new(Text.new, attributes: { 'key' => 'value' })
  ...
  describe '==' do
    it "returns false if elements attributes don't match" do
      element1 = Element.new(Text.new, attributes: { 'key' => 'value' })
      element2 = Element.new(Text.new, attributes: { 'key' => 'other_value' })
  ...
  describe '==' do
    it "returns false if elements attributes don't match" do
      element1 = Element.new(Text.new, attributes: { 'key' => 'value' })
      element2 = Element.new(Text.new, Text.new, attributes: { 'key' => 'value' })
  ...
  describe '#max_occurs' do
    ...
    it "returns value from maxOccurs if it's set" do
      element = Element.new(attributes: { 'maxOccurs' => '4' })

      expect(element.max_occurs).to eq 4
    end

    it "returns Infinity if maxOccurs is set to 'unbounded'" do
      element = Element.new(attributes: { 'maxOccurs' => 'unbounded' })
  ...
  describe '#multiple?' do
    it "returns true if element's maxOccurs is bigger than 1" do
      element = Element.new(attributes: { 'maxOccurs' => '2' })

      expect(element.multiple?).to eq true
    end

    it "returns false if element's maxOccurs is 1" do
      element = Element.new(attributes: { 'maxOccurs' => '1' })
